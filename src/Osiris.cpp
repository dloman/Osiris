...
std::deque<cv::Mat> gFrames;

FilteredCircularBuffer<10> gBoundingBoxBuffer;
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void ProcessImage(cv::Mat& VideoFrame)
{
  ...
  AveragedFrame = AveragedFrame / gFrames.size();
  ...
  if (LinePoints.size())
  {
    ...
    auto DidItWork = gBoundingBoxBuffer.push_back(Temp);
  ...
  auto CurrentTablePosition = gBoundingBoxBuffer.GetValue();
  ...
  std::cout << "Table Score = " << ComputeScore(ColorHistogram) << '\n';
